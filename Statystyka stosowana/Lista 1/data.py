...
class ContinuousData(Data):
    ...
    def pdf_lognormal(self,mu,variance,show:bool,hist:bool,bins=15):
        dane = np.array(self._hist(show=hist,bins=bins))
        y = 1/(np.sqrt(2*np.pi)*variance*dane)*np.exp(-0.5*((np.log(dane)-mu)/variance)**2)
        plt.plot(dane,y,color='r')
        self._show_plot(show)
...
if __name__ == "__main__":
    T = 10
    dane = ContinuousData(np.random.normal(10,2,100000))
